...
Настройте конфигурацию сервиса `ADB` в соответствии с вашими требованиями. Включите отображение `advanced` параметров.
Подробная информация про конфигурационные параметры `ADB` в [документации](https://docs.arenadata.io/ru/ADB/current/references/adcm-config.html)
Наиболее важные конфигурационные параметры:

|  Раздел  |           Параметр          |                               Описание                                 |
| -------- | --------------------------- | ---------------------------------------------------------------------- |
|   Main   | Name of default database.   | Название базы данных создаваемой по-умолчанию                          |
|   Main   | Use segment mirroring.      | Включить ли зеркалирование сегментов. По-умолчанию `true`              |
| Advanced | Number of segments per host | Количество сегментов на каждый хост. По-умолчанию это кол-во `ЦПУ / 2` |

На двух хостах выделенных под сегменты `dwh-adb-segment1` и `dwh-adb-segment2` по 8 ядер. То есть на каждом будет создано по 4 `primary` сегмента. Итого 8 `primary` сегментов. Помимо этого будет создан один `replica` сегмент на каждый `primary` сегмент. Итого 16 сегментов: 8 `primary` и 8 `replica` на двух хостах.

### Шаг 10. Подключение к ADB Control 
